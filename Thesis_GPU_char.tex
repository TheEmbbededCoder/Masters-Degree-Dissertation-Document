...
For the memory tests (\acrshort{dram}, Cache and Shared Memory), only integer data type was used since it was easier to assess if no data corruption had happened and the effects on memory are the same for floating-point operation.
...
The devised benchmarks were applied to characterize the AMD Vega 10 Frontier Edition and AMD Radeon 5700 XT \acrshort{gpu}s, whose specifications are presented in Table II~\ref{tab:Vega10specs}. 

\begin{table}[htbp]
    \caption{\acrshort{gpu}s under test specifications.}
    \begin{center}
         \resizebox{\textwidth}{!}{%
            \begin{tabular}{llcc}
...
\begin{figure}[!htb]
    ...
    \begin{subfigmatrix}{2}
      ...
      \subfigure[Radeon 5700 XT.]{
        \centering\includegraphics[width=0.42\textwidth]{Figures/GPU_characterization/5700XT-CacheL2_Guardband.pdf}
...
\begin{figure}[!htb]
    ...
    \begin{subfigmatrix}{2}
      ...
      \subfigure[Vega 10.]{
        \centering
        \includegraphics[width=0.95\textwidth]{Figures/GPU_characterization/MAC_Guardband.pdf}
        \label{fig:Vega10-MAC_guardband}
      }
      \subfigure[Radeon 5700 XT.]{
        \centering\includegraphics[width=0.95\textwidth]{Figures/GPU_characterization/5700XT-MAC_Guardband.pdf}
...
Figure~\ref{fig:Guardband_comparison} presents a comprehensive comparison of the valid voltage ranges for all the considered architectural components of both \acrhosrt{gpu}s. The general motive is that the CacheL2 and the \acrshort{alu} are the two components that compromise the undervoltage capabilities. CacheL2 affects kernels that are more memory intensive, while the \acrshort{alu} limits those that are more compute-intensive. 
...
In general, even though two completely different \acrhosrt{gpu} architectures are under test, the general behaviour of the both is similar. At the lower frequencies available, the minimum voltage can be safely applied without the existence of computation errors or crashes. The utilization of undervoltage for higher frequencies will depend on which components the application being executed stresses the most. Across all frequencies, both \acrhosrt{gpu}s allow for more than $20\%$ of save undervoltage, being a considerable amount to be explored on the following section.
...
\begin{figure}[!htb]
    ...
    \begin{subfigmatrix}{2}
      ...
      \subfigure[Radeon 5700 XT.]{
        \centering\includegraphics[width=0.75\textwidth]{Figures/GPU_characterization/5700XT-MAC_EDP.pdf}
