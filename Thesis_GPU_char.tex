...
The devised benchmarks were applied to characterize the AMD Vega 10 Frontier Edition and AMD Radeon 5700 XT \acrshort{gpu}s, whose specifications are presented in Table~\ref{tab:Vega10specs}. 
...
Figure~\ref{fig:DRAM_guardband} illustrates the usable voltage range of the DRAM domain on the Vega 10 \acrhosrt{gpu}. Across the complete, tested frequency range, it is possible to undervolt the memory to $800mV$, independently of the value of \texttt{OPS} (parameter variation between 0 and 50 operations, see Listings~\ref{lst:DRAMbench}).
...
For the Radeon 5700 XT, similar behavior is found. However, even at the test with \texttt{OPS}=40 the limiting factor is still the Shared Memory. This is due to a reduced memory interface and maximum memory bandwidth of the Radeon 5700 XT (256-bit and 448GB/s) card versus the Vega 10 (2048-bit and 484GB/s).
...
Figure~\ref{fig:cache_edp} and \ref{fig:sm_edp} presents the obtained \acrshort{edp} for the Cache and shared memory benchmark. These components favor the utilization of minimum voltages to achieve an EDP improvement of over $40\%$.
...
\begin{figure}[!htb]
    ...
    \begin{subfigmatrix}{2}
      ...
      \subfigure[Vega 10.]{
        \centering
        \includegraphics[width=0.53\textwidth]{Figures/GPU_characterization/SM_EDP_OPS0.pdf}
        \label{fig:Vega10-sm_edp}
      }
      \subfigure[Radeon 5700 XT.]{
        \centering\includegraphics[width=0.37\textwidth]{Figures/GPU_characterization/5700XT-SharedMemory_EDP_0OP.pdf}
