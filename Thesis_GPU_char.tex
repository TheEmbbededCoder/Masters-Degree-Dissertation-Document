...
Figure~\ref{fig:DRAM_Corebehaviour} illustrates the normalized performance and energy consumption, again when varying the \texttt{OPS} parameter between 0 and 50 operations (see Listings~\ref{lst:DRAMbench}) when scaling V-F dor the \textit{Core} \acrshort{dvfs} domain. The experiment shows that for memory bounded kernels (\texttt{OPS} values $0$ and $10$), performing frequency scaling does not change the computation time. However, it significantly impacts energy consumption (going from the highest frequency to the lowest yields a $45\%$ reduction on energy consumption). For the case of a kernel that is not memory bounded  (\texttt{OPS} $=25$), the first benefit of exploring non-conventional V-F pairs emerges. For this test, the traditional V-F pairs (connected by the dashed line on Figure~\ref{fig:DRAM_Corebehaviour})  can already yield a $41\%$ energy consumption reduction aggregated with a $15\%$ performance increased. However, by exploring the non-conventional V-F pairs, it is possible to improve both metrics further. Running the \acrshort{gpu} at $1600MHz$ and $1000mV$ yields an $18\%$ performance increase while running \acrshort{gpu} at $1600MHz$ and $1000mV$ allows a $58\%$ energy reduction with a $14\%$ performance increase.
...
\begin{figure}[!htb]
  \begin{subfigmatrix}{2}
    \subfigure[Cache]{\includegraphics[width=0.5\textwidth]{Figures/GPU_characterization/CacheL2_behaviour_0op.pdf}}
    \subfigure[Shared Memory]{\includegraphics[width=0.48\textwidth]{Figures/GPU_characterization/SharedMemory_behaviour.pdf}}
  \end{subfigmatrix}
...
% \begin{figure}[htb]
%   \centering
%   \includegraphics[width=0.8\textwidth]{Figures/GPU_characterization/CacheL2_behaviour_0op.pdf}
%   \caption{Core domain - Cache - Normalized energy and performance variations for \texttt{OPS}=0, when applying V-F scaling to the GPU Core. The  values shown in the figure represent the applied core voltage and the dashed line connects the default F-V configurations.}
%   \label{fig:cacheL2_behaviour}
% \end{figure}
...
% \begin{figure}[htb]
%   \centering
%   \includegraphics[width=0.8\textwidth]{Figures/GPU_characterization/SharedMemory_behaviour.pdf}
%   \caption{Core domain - Shared Memory - Normalized energy and performance variations for \texttt{OPS}=0, when applying V-F scaling to the GPU Core. The dashed line connects the default F-V configurations.}
%   \label{fig:cacheL2_behaviour}
% \end{figure}
...
\begin{figure}[htb]
  ...
  \caption{Core domain - ALU-MAC - Normalized energy and performance chart for the benchmark setup with different values of \texttt{d} (see Listing~\ref{lst:MACbench}). The dashed lines connect the results for default F-V configurations.}
